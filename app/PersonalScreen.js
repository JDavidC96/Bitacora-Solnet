...
export default function PersonalScreen() {
  ...
  // --- Asignaciones específicas (existentes) ---
  ...
  const handleAssignToRetie = async (person) => {
    setLoading(true);
    try {
      await personalService.assignToDestination(person.id, "Visita RETIE");
      setSelectedPerson(null);
  ...
  const handleAssignToOffice = async (person) => {
    setLoading(true);
    try {
      await personalService.assignToDestination(person.id, "Oficina");
      setSelectedPerson(null);
  ...
  /**
   * NEW: Handler genérico para los nuevos destinos
   * (Dirección de Operaciones, Diseño de Planos, Monitoreo, etc.)
   */
  const handleAssignToDestination = async (person, destino) => {
    setLoading(true);
    try {
      await personalService.assignToDestination(person.id, destino);
      setSelectedPerson(null);
  ...
  const handleAssignManual = async (person, destino) => {
    ...
    setLoading(true);
    try {
      await personalService.assignToDestination(person.id, destino.trim());
      setSelectedPerson(null);
      Alert.alert("Éxito", `${person.nombre} asignado a ${destino}`);
    } catch (error) {
      Alert.alert("Error", error.message || "No se pudo asignar");
  ...
  const handleExportExcelPersona = async (person) => {
    ...
    try {
      ...
      if (!registros || registros.length === 0) {
        Alert.alert("Exportación", "Esta persona no tiene registros aún.");
        return;
      }
      ...
      if (!ok) {
        Alert.alert("Error", message || "No se pudo exportar el Excel");
      }
    } catch (error) {
  ...
  return (
    ...
    >
      ...
      <PersonalActionsModal
        ...
        onAssignToDestination={handleAssignToDestination}
